...
def load_overall_analysis():
    ...
    with col1:
        ...
        selected_option = st.selectbox(
            "Select Type", ["Total", "Count", "All Total", "All Count"]
        )
    ...
    with col4:
        ...
        data = (
            df.groupby("startup", as_index=False)["amount"]
            .sum()
            .sort_values("amount", ascending=False)[["startup", "amount"]]
        )
    ...
    data = (
        df.groupby("investors")["amount"].sum().sort_values(ascending=False).head(100)
    )
    st.dataframe(data)
    ...
    sns.heatmap(
        grouped_data.to_frame().T, ax=ax, cmap="YlGnBu"
...
def load_investor_detail(investor):
    ...
    try:
        ...
        with col6:
            ...
            matching_verticals = np.intersect1d(
                (investor["vertical"]), (df["vertical"].unique())
            )
            v = list(matching_verticals)
            # Filter based on matching verticals
            filtered_df = df[df["vertical"].isin(v)][
                ["startup", "vertical", "investors"]
            ].set_index("vertical")
...
def company_details(name):
    ...
    details = temp_df[temp_df["startup"] == name][
    ...
    v_list = list(v)
    filtered_df = temp_df[temp_df["subVertical"].isin(v_list)][
        ["startup", "subVertical", "date"]
    ].set_index("startup")
